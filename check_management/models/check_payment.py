...
class normal_payments(models.Model):
    ...
    send_rec_money = fields.Selection(string="Payment Type",selection=[('send','Send Money'),('rece','Receive Money')])
    ...
    def filter_journals(self):
        if self.send_rec_money:
            if self.send_rec_money == 'send':
                return {
                    'domain': {'payment_method': [('payment_subtype', '=', 'issue_check')]}
                }
            else:
                return {
                    'domain': {'payment_method': [('payment_subtype', '=', 'rece_check')]}
                }

    @api.onchange('payment_method')
    def check_if_payment_type_valid(self):
        if not self.send_rec_money and self.payment_method:
            raise exceptions.ValidationError('You must select payment type first')
        else:
            pass
